...
"""ICHOR (Inference of the primary Case in Hepatitis C OutbReaks), by J Walker Gussler
Given a number of .fas clinical HCV samples of HVR1 sequenced by the GHOST protocol, this program will determine the presence of the primary case which is most likely to engage in super-spreader activity
...
# replace kstep routine with another -> I doubt this will save very much time
# replace pre_align (it's kinda broken idk)(aligning files in pairwise fashion produces strange, undesirable results)
# check input for duplicate sequences (will error out in combineseqs there are any)
# make allcaps variables global rather than passing them around? (I was getting an annoying report whenever i ran the program about declaration)
# parsing efficiency? (dont parse for each method) (is it worth the memory storage?)
    # tough - we don't want to calculate features on 100 seq reduced sets, do we?
# ~the pavel code~
# simul_evol is very gross (several reasons)
...
def calc_ordered_frequencies(haploNum,haploSize,seqs,byFreq): #split into 2 programs? if byFreq==True, seqs must be dict[seq]=freq but if byFreq==False, seqs must be list
    #get_evol_time
...
def get_intermediate_sequences(kStepList,seqs,haploSize): #make 1-step list of sequences based off k-step list, return list of sequences
    ...
    finalSeqs.extend(list(fakeSeqs.values()))
    return finalSeqs
...
def simul_evol(D_all,nseq1,nseq2,len_eff,PERCENT,FAILTIME): #viral evolution simulator, works, probably could be changed
    ...
    for u in range(sz):
        Q[u,u]=(mutprob/3)**D_all[u,u]*(1-mutprob)**(len_eff-D_all[u,u])            #modifying this line? vvvvvvvvvvvvv
...
def combine_seqs(seqs1,seqs2):
    #helper function for get_evol_times
    # for seq in seqs1:
        # print(seq)
    # print('-')
    # for seq in seqs2:
...
def calc_distance_matrix_slow(finalSeqs): 
    l=len(finalSeqs)
    arr=np.zeros([l,l])
...
def outdegree_source(DSamp,files): #reutrns dictionary with each file as key and evolution factor as value
    #main
    times={}
...
def source_from_features(INPUTS,ALI,NOGHOST): # determine source from list of features that we calculate
    #main
    predictions={}
    ...
    for file in INPUTS:
        ...
        # calculate kemr entropy (kmerEntropy)
        kmers=get_kmers(seqs,8)
        kv=list(kmers.values())
        totalKmers=float(sum(kv))
        newkmers=[]
        for i in kv:
            freq=i/totalKmers
            newval=freq*math.log(freq)
        ...
        print(','.join(list(map(str,[file,kmerEntropy,nucDiv,nucEntropy,maxHamming,haploFreq]))))
...
###################pavel area####################################
######tread lightly ye who enter here############################
###################pavel area####################################
    ...
    if type(probD)!=bool:
        for row in range(numRows):
            for col in range(numCols):
                if row!=col:
                    # if probD[row,col]>.20 or transnet[row,col]!=0:
                    if probD[row,col]>.20:
...
def render_with_graphviz(transnet,fileList,OUTPUT,strength,probD,expect): 
    ...
    if strength==False:
        ...
        with open(tmpdotname,'r') as f:
            lines=f.readlines()
        l=len(lines)
        with NamedTemporaryFile(mode='w', delete=False) as outdot: 
...
def obj_transnet_phylo_fit(AMtree,tree,DM): #calculates likelihood of given genetic distance given a possible transmission tree using a least-square approach
    #find_transnet_MCMC
    #often returns 0 - I thought this was a bug, but then I ran the same input on pavel's version and it also gave 0's throughout
    #this sometimes affects the output in that we won't have probabilities on the edges (we only ever took one tree)
    #software handles this pretty quietly, however i HATE this program tbh
...
def cluster_num(X,T,k,c): #takes in matrix already added
    m=len(X)+1
...
def find_transnet_MCMC(DSamp_mcmc,FAILTIME): #this is the second 'wrapper' part of the program - iteratively constructs most likely transmission network
    #inferTransNetFromEvolTimes
    #the bit that is commented out is probably not necessary, idk
...
def print_components(DSamp,INPUTS,S,C): #complain and print (more than two connected components via ICHOR metric inside hamming clique) (rare)
    #main
    #pavels code was made to be able to handle files which arent related, this is sort of a remnant of that
    for a in range(S):
        ...
        if len(runlist)>1:
            rerun="/path/to/ichor.py "
            for item in runlist:
                rerun=rerun+" "+item
            print(rerun)
        else:
            print("%s does not appear to be connected to any of the other files" % runlist[0])
    sys.exit("exiting ICHOR...")        

def main(INPUTS,ALI,FAILTIME,PERCENT,NOGHOST,OUTPUT,CLUSTERS):
    print('Welcome to ICHOR! Running evolutionary simulations...')
    ...
    for input in INPUTS:
        ...
        # else:
            # try:
                # seqsDict[input]=calc_centroids(input)
            # except:
                # sys.exit("Something went wrong while I was trying to read "+input+". Exiting")
...
##############################################################################################################################################
    ...
    for row in DSamp:
        print(row)
    [S,C]=determine_components(DSamp) #S==number of connected components in the graph (number of outbreaks by ICHOR evolutionary metric)        
    ...
    for item in outdegrees:
        if outdegrees[item]==maxout:
            if item==timeSource:
                fail=False
    if fail==True:
        sys.exit('Error with evolutionary simulations: highest outdegree != highest evolutionary factor. Exiting')
    # print(timeSource,featureSource)
    workTime =  time.time() - startTime
    if timeSource==featureSource:
        expect=timeSource
        source,strength=figure(times,predictions,INPUTS,timeSource)
        statement_time="ICHOR analysis: LOW CONFIDENCE PREDICTION! %s is the predicted source. Analysis took %.3f seconds. Thank you for using ICHOR!" % (source, workTime)
        if strength==True:
            print('Among the given samples, %s appears to be the most likely candidate for the source' % source)
        statement_time='ICHOR analyis was successful! Analysis took %.3f seconds. Thank you for using ICHOR!' % workTime
        ...
        statement_time='ICHOR analysis: source undeterminable or not present - feature source != evolution source! Analysis took %.3f seconds. Thank you for using ICHOR!' % workTime
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='ICHOR: predict directionality of HCV infection')
    ...
    main(INPUTS,ALI,FAILTIME,PERCENT,NOGHOST,OUTPUT,CLUSTERS)
